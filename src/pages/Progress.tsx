...
export default function ProgressPage() {
  ...
  const progressToTarget = startWeight && target && currentWeight 
    ? Math.min(((startWeight - currentWeight) / (startWeight - target)) * 100, 100)
    : 0;
  ...
  const getWeeklyChartData = () => {
    ...
    const weeklyData = weeks.slice(-12).map(weekDate => {
      ...
      const avgWeight = logsInWeek.length > 0 
        ? logsInWeek.reduce((sum, log) => sum + log.weight, 0) / logsInWeek.length 
        : null;
              ...
              <div className="text-center space-y-3">
                ...
                {weightLogs.length > 1 && (
                  <Badge className="bg-blue-500 text-white">
                    עדכון אחרון: {format(new Date(weightLogs[0].log_date), 'dd/MM/yyyy')}
                  </Badge>
                )}
              </div>
            </CardContent>
                  ...
                  <>
                    ...
                    {target && currentWeight && (
                      <Badge className="bg-purple-500 text-white">
                        נשאר: {remainingWeight.toFixed(1)} ק"ג
                      </Badge>
                    )}
                  </>
                )}
                  ...
                  <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="weight" radius={[12, 12, 0, 0]} maxBarSize={60}>
                      {chartData.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
                          fill={index === chartData.length - 1 ? '#10b981' : '#60a5fa'} 
                        />
                      ))}
                    </Bar>
                  </BarChart>
              ...
              >
                <CardContent className="space-y-4 border-t pt-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="newWeight" className="text-lg font-bold">משקל (ק"ג)</Label>
                      <Input 
                        id="newWeight"
                        type="number" 
                        step="0.1"
                    ...
                    <div>
                      <Label htmlFor="weightDate" className="text-lg font-bold">תאריך השקילה</Label>
                      <Input 
                        id="weightDate"
                        type="date" 
